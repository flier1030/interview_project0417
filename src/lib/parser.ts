...
export function getTopLevelReadableElementsOnPage(): HTMLElement[] {
  const allEments = Array.from(document.body.querySelectorAll<HTMLElement>('p, blockquote, li'));
  ...
  const readableElements = allEments.filter(el => {
    ...
    const parentReadable = el.closest('p, blockquote, li');
    return parentReadable === el;
  })
