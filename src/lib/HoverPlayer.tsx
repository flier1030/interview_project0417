...
export default function HoverPlayer() {
  let parseElementRefs = useRef<HTMLElement[]>([]);

  useEffect(() => {
    parseElementRefs.current = getTopLevelReadableElementsOnPage();
  }, []);

  const hoverInfo = useHoveredParagraphCoordinate(parseElementRefs.current);
